...
DriveMeterProcessor::DriveMeterProcessor()
...
void DriveMeterProcessor::prepare(float sr)
{
...
void DriveMeterProcessor::setAttackMs(float ms)
{
    attackMs = ms;
    updateCoeffs();
}

void DriveMeterProcessor::setReleaseMs(float ms)
{
    releaseMs = ms;
    updateCoeffs();
}

float DriveMeterProcessor::processSample(float x)
{
...
float DriveMeterProcessor::getNormalizedLevel()
{
...
int DriveMeterProcessor::getLedSegmentIndex(int segments = 8)
{
    return int(getNormalizedLevel() * (segments - 1) + 0.5f);

}

bool DriveMeterProcessor::isStandardSampleRate(float sr)
{
    switch (int(sr)) {
...
void DriveMeterProcessor::updateCoeffs()
{
